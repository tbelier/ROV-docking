...
#include "subsonus_pkg/msg/range.hpp"
#include "subsonus_pkg/msg/usblmain.hpp"
...
public:
  ...
  subsonus_pkg::msg::Range msg_range_azimut = subsonus_pkg::msg::Range();
  subsonus_pkg::msg::Usblmain msg_main = subsonus_pkg::msg::Usblmain();
  ...
  : Node("talker"), count_(0)
  {
    ...
    publisher_range = this->create_publisher<subsonus_pkg::msg::Range>("range_sub", 1000);
    publisher_main = this->create_publisher<subsonus_pkg::msg::Usblmain>("main_usbl", 1000);
...
private:
  ...
  {  
    ...
    // msg_main.header.stamp=this->get_clock()->now();
    ...
    FD_ZERO(&readfds);
		...
		if(FD_ISSET(tcp_socket, &readfds))
		{
          ...
          // printf("test %u\n",an_packet->id);
              ...
              // Yaw (Z-axis rotation)
              msg_main.yaw_surface = system_state_packet.orientation[2]*180/M_PI;
              ...
              // Filtrage des données de positions d'un USBL par rapport à l'autre : 
              // Listes des données
              ...
              msg_range_azimut.range = subsonus_track_packet.range;
              msg_range_azimut.azimuth = subsonus_track_packet.azimuth*RADIANS_TO_DEGREES;
              msg_range_azimut.elevation = subsonus_track_packet.elevation*RADIANS_TO_DEGREES;
              publisher_range->publish(msg_range_azimut);


              double median_range;
              double median_azimuth;

              // Filtrage des données range, azimut : 
              ...
              msg_main.dx = median_pos_x;
              msg_main.dy = median_pos_y;
              msg_main.dz = median_pos_z;
              msg_main.range = median_range;
              msg_main.azimut = median_azimuth;
            }
					}
            ...
            // Yaw (Z-axis rotation)
            msg_main.yaw_rov = subsonus_remote_system_state_packet.orientation[2]*180/M_PI;
    ...
    // publisher_sub->publish(message_sub);
    // publisher_range->publish(msg_range_azimut);
    publisher_main->publish(msg_main);
  ...
  rclcpp::Publisher<subsonus_pkg::msg::Range>::SharedPtr publisher_range;
  rclcpp::Publisher<subsonus_pkg::msg::Usblmain>::SharedPtr publisher_main;
