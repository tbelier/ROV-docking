...
if __name__ == "__main__":
    sizeMarker = 0.164
    cap = cv2.VideoCapture(0) # cv2.VideoCapture("saves/airEauDetect.avi")
    ...
    while True:
        ...
        cv_image = frame#image_treatment(frame)

        detected_image, rvecs = detect(cv_image, sizeMarker)
        if not(isinstance(rvecs, int)):  print(rvecs.astype(int))
